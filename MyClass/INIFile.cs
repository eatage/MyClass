...
namespace MyClass
{
    ...
    class INIFile
    {
        ...
        public static Boolean DelSection(string section, string as_FilePath = "")
        {
            StringBuilder temp = new StringBuilder(1024);
            if (as_FilePath == "")
            {
                if (WritePrivateProfileString(section, null, null, strFilePath)<=0)
                {
            ...
            {
                if (WritePrivateProfileString(section, null, null, as_FilePath)<=0)
                {
                    //throw (new ApplicationException("无法清除Ini文件中的Section"));
                    return false;
                }
